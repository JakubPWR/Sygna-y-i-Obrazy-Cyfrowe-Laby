...
def calculate_mse(original, modified):
    # Dopasowanie rozmiarów obrazów przed obliczeniem MSE
    modified_resized = np.array(
        Image.fromarray(modified.astype(np.uint8)).resize((original.shape[1], original.shape[0])))
    return np.mean((original - modified_resized) ** 2)
...
# Obliczanie MSE między oryginalnym a wynikowym obrazem po max pooling
mse_max_pooling = calculate_mse(image_matrix, result_matrix)
print(f"MSE after Max Pooling: {mse_max_pooling}")
...
downsampled_matrix = downsample(image_matrix, scale_factor=2)

# Obliczanie MSE między oryginalnym a zeskalowanym obrazem
mse_downsample = calculate_mse(image_matrix, downsampled_matrix)
print(f"MSE after Downsample: {mse_downsample}")
...
upsampled_matrix = upsample_nearest_neighbor(downsampled_matrix, scale_factor=2)
